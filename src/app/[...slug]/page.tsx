...
async function getSiteData(slug: string) {
  ...
  const [monthlyResult] = await db
    .select({ total: sql<number>`coalesce(sum(${hits.count}), 0)` })
    .from(hits)
    .where(and(eq(hits.siteId, site.id), gte(hits.date, monthAgo.toISOString().split("T")[0])));
  ...
  const countries = await db
    .select({ country: hitLogs.country, count: sql<number>`count(*)` })
    .from(hitLogs)
  ...
  const devices = await db
    .select({ device: hitLogs.device, count: sql<number>`count(*)` })
    .from(hitLogs)
    .where(eq(hitLogs.siteId, site.id))
  ...
  const referers = await db
    .select({ referer: hitLogs.referer, count: sql<number>`count(*)` })
    .from(hitLogs)
